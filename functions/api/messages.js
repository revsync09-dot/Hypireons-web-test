...
export async function onRequest(context) {
    ...
    try {
        ...
        const [msgRows, vcRows, growthRows, allMessageRows, allVcRows] = await Promise.all([
            ...
            fetchAllRows(supabase, 'server_messages', 'user_id, channel_id, deleted, sent_at', (q) => q.eq('guild_id', guildId).order('sent_at', { ascending: true })),
            fetchAllRows(supabase, 'vc_sessions', 'user_id, username, joined_at, left_at, duration_minutes', (q) => q.eq('guild_id', guildId).order('joined_at', { ascending: true }))
        ]);
        ...
        let totalMessages = 0, totalDeleted = 0;
        const summaryUsers = new Set(), summaryChannels = new Set();
        for (const row of allMessageRows) {
            if (row.deleted) totalDeleted++;
            else {
                totalMessages++;
        ...
        for (const row of allVcRows) {
            if (row.user_id) summaryUsers.add(row.user_id);
        }

        const totalVcMinutes = allVcRows.reduce((s, r) => s + resolveVcDurationMinutes(r, now), 0);
        const topUserAvatar = topUsers[0]?.avatar || getDefaultAvatar(topUsers[0]?.userId || '0');

        return new Response(JSON.stringify({
            range,
            meta: { generatedAt: now.toISOString(), bucketSizeMinutes: Math.round(bucketSizeMs / 60000), bucketCount },
            summary: { totalMessages, totalDeleted, totalVcMinutes: Math.round(totalVcMinutes), totalVcHours: Math.round(totalVcMinutes / 60), activeChannels: summaryChannels.size, activeUsers: summaryUsers.size, topUserAvatar },
