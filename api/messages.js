...
module.exports = async (req, res) => {
  ...
  const emojis = {
    messages: process.env.EMOJI_MESSAGES || '💬',
    deleted: process.env.EMOJI_DELETED || '🗑️',
    vc: process.env.EMOJI_VC || '🔊',
    channels: process.env.EMOJI_CHANNELS || '#',
    users: process.env.EMOJI_USERS || '👥',
    chart: process.env.EMOJI_CHART || '📈',
    wave: process.env.EMOJI_WAVE || '🌊',
    activity: process.env.EMOJI_ACTIVITY || '📊',
    growth: process.env.EMOJI_GROWTH || '📈',
    joins: process.env.EMOJI_JOINS || '📈',
    leaves: process.env.EMOJI_LEAVES || '📉',
    text: process.env.EMOJI_TEXT || '💬',
    boardUsers: process.env.EMOJI_BOARD_USERS || '👤',
    boardChannels: process.env.EMOJI_BOARD_CHANNELS || '#',
    boardVc: process.env.EMOJI_BOARD_VC || '🎙️'
  };
  ...
  try {
    ...
    const now = new Date();
    const tzOffsetMinutes = Number(req.query.tzOffsetMinutes);
    const tzOffsetMs = Number.isFinite(tzOffsetMinutes) ? tzOffsetMinutes * 60000 : 0;
    ...
    const [msgRows, vcRows, growthRows, allMessageRows, allVcRows] = await Promise.all([
      ...
      fetchAllRows(
        supabase,
        'server_messages',
        'user_id, channel_id, deleted, sent_at',
        (query) => query
          .eq('guild_id', guildId)
          .order('sent_at', { ascending: true })
      ),
    ...
    const totalVcMinutes = allVcRows.reduce((sum, row) => sum + resolveVcDurationMinutes(row, now), 0);
    ...
    res.json({
      ...
      debug: {
        ...
        totalStoredMessages: allMessageRows.length,
