...
function drawGlowOrb(ctx, x, y, radius, color, alpha = 0.22) {
  ...
  ctx.save(); ctx.fillStyle = g; ctx.fillRect(x - radius, y - radius, radius * 2, radius * 2); ctx.restore();
}
function drawAvatar(ctx, img, cx, cy, radius, ringColorA, ringColorB, glowColor) {
  ctx.save();
  ctx.strokeStyle = `${glowColor}18`; ctx.lineWidth = 1;
  for (const extra of [52, 36]) {
  ...
  ring.addColorStop(0, ringColorA); ring.addColorStop(1, ringColorB);
  ctx.strokeStyle = ring; ctx.lineWidth = 4;
  ctx.beginPath(); ctx.arc(cx, cy, radius + 4, 0, Math.PI * 2); ctx.stroke();
  ctx.restore();
  ctx.save();
  ctx.strokeStyle = 'rgba(255,255,255,0.3)'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.arc(cx, cy, radius - 2, -Math.PI * 0.85, -Math.PI * 0.2); ctx.stroke();
  ctx.restore();
}
function drawStar(ctx, cx, cy, radius, filled, colA, colB) {
  ...
  if (filled) {
    ...
    ctx.save(); ctx.shadowColor = colB; ctx.shadowBlur = 18; ctx.fillStyle = g; ctx.fill(); ctx.restore();
  } else {
...
function drawGrain(ctx, W, H, amount = 3000, alpha = 0.05) {
  ...
  for (let i = 0; i < amount; i++) {
    const v = Math.random() > 0.5 ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.9)';
    ctx.fillStyle = v;
...
async function loadGameEmoji(gameKey) {
  ...
  const emoji = map[String(gameKey).toUpperCase()];
...
async function buildVouchCard(data) {
  ...
  const t = getTheme(data.gameKey);
  const bg = ctx.createLinearGradient(0, 0, W, H);
  bg.addColorStop(0,    '#030508');
  bg.addColorStop(0.35, t.dark);
  bg.addColorStop(0.7,  '#040609');
  ...
  drawHexGrid(ctx, W, H);
  drawDotGrid(ctx, W, H, 'rgba(255,255,255,0.025)', 32);
  drawGrain(ctx, W, H, 4000, 0.035);
  drawGlowOrb(ctx, 250,   200,   600, t.A,       0.18);
  drawGlowOrb(ctx, W - 300, H - 160, 700, t.B,   0.14);
  drawGlowOrb(ctx, W * 0.5, H * 0.48, 900, '#ffffff', 0.025);
  drawGlowOrb(ctx, W * 0.7, 120,   400, t.glow,  0.12);
  ctx.save();
  ctx.shadowColor = 'rgba(0,0,0,0.8)'; ctx.shadowBlur = 80; ctx.shadowOffsetY = 40;
  fillRoundRect(ctx, 30, 24, W - 60, H - 48, 40, 'rgba(8,10,18,0.85)');
  ctx.restore();
  ...
  shellGrad.addColorStop(0,    'rgba(255,255,255,0.22)');
  shellGrad.addColorStop(0.3,  `${t.A}66`);
  shellGrad.addColorStop(0.7,  `${t.B}33`);
  shellGrad.addColorStop(1,    'rgba(255,255,255,0.06)');
  strokeRoundRect(ctx, 30, 24, W - 60, H - 48, 40, shellGrad, 1.5);
  ctx.save();
  roundRect(ctx, 30, 24, W - 60, H - 48, 40); ctx.clip();
  const shine = ctx.createLinearGradient(60, 40, 900, 550);
  shine.addColorStop(0,    'rgba(255,255,255,0.12)');
  shine.addColorStop(0.15, 'rgba(255,255,255,0.04)');
  shine.addColorStop(0.3,  'transparent');
  ctx.fillStyle = shine; ctx.fillRect(30, 24, 900, 500);
  const headerBar = ctx.createLinearGradient(60, 0, W * 0.5, 0);
  headerBar.addColorStop(0, t.A); headerBar.addColorStop(1, 'transparent');
  ctx.fillStyle = headerBar; ctx.fillRect(60, 54, 700, 3);
  ctx.textAlign = 'left';
  ctx.fillStyle = 'white';
  ...
  const sepLine = ctx.createLinearGradient(62, 0, 900, 0);
  sepLine.addColorStop(0, t.A); sepLine.addColorStop(0.6, `${t.B}66`); sepLine.addColorStop(1, 'transparent');
  ctx.fillStyle = sepLine; ctx.fillRect(62, 128, 840, 3);
  ctx.restore();
  const PAD = 24;
  ...
  for (const [px, pw] of [[leftX, COL_LEFT_W], [midX, COL_MID_W], [rightX, COL_RIGHT_W]]) {
    fillRoundRect(ctx, px, PANEL_Y, pw, PANEL_H, PANEL_RADIUS, 'rgba(255,255,255,0.025)');
    ctx.save();
    roundRect(ctx, px, PANEL_Y, pw, PANEL_H, PANEL_RADIUS); ctx.clip();
    const panelTint = ctx.createLinearGradient(px, PANEL_Y, px, PANEL_Y + 80);
    panelTint.addColorStop(0, `${t.A}18`); panelTint.addColorStop(1, 'transparent');
    ctx.fillStyle = panelTint; ctx.fillRect(px, PANEL_Y, pw, 80);
    ctx.restore();
    strokeRoundRect(ctx, px, PANEL_Y, pw, PANEL_H, PANEL_RADIUS, 'rgba(255,255,255,0.08)', 1);
    ctx.save();
  ...
  const serviceImg   = await loadGameEmoji(data.gameKey);
  ...
  ctx.fillText(`${String(data.gameKey || 'HELPER').toUpperCase()} HELPER`, hcx, PANEL_Y + 325);
  const gameTag = String(data.gameLabel || data.gameKey).toUpperCase();
  ...
  const statCards = [
    ...
    { label: 'TOP SERVICE',    value: String(data.stats.topGame || data.gameKey || '—'),  sub: 'Primary game',           color: '#ffffff' }
  ];
...
async function buildHelperProfileCard(data) {
  ...
  const bgG = ctx.createRadialGradient(W * 0.5, H * 0.4, 0, W * 0.5, H * 0.5, W);
  bgG.addColorStop(0, t.mid);
  bgG.addColorStop(1, t.dark);
  ctx.fillStyle = bgG;
  ctx.fillRect(0, 0, W, H);
  drawHexGrid(ctx, W, H);
  drawDotGrid(ctx, W, H);
  drawGlowOrb(ctx, W * 0.2, H * 0.4, 600, t.A, 0.18);
  drawGlowOrb(ctx, W * 0.8, H * 0.6, 500, t.B, 0.12);
  drawGrain(ctx, W, H, 3500, 0.035);
  const P_MARGIN = 40;
  fillRoundRect(ctx, P_MARGIN, P_MARGIN, W - P_MARGIN * 2, H - P_MARGIN * 2, 38, 'rgba(8,11,20,0.4)');
...
async function buildLeaderboardCard(data) {
  ...
  const bgG = ctx.createRadialGradient(W / 2, H / 3, 0, W / 2, H / 2, Math.max(W, H));
  bgG.addColorStop(0, t.mid); bgG.addColorStop(1, t.dark);
  ctx.fillStyle = bgG; ctx.fillRect(0, 0, W, H);
  drawHexGrid(ctx, W, H); drawDotGrid(ctx, W, H);
  drawGlowOrb(ctx, W / 2, 80, 600, t.A, 0.18);
  drawGrain(ctx, W, H, 2000, 0.03);
...
async function buildBotStatusCard(data) {
  ...
  ctx.fillStyle = bgG; ctx.fillRect(0, 0, W, H);
  drawHexGrid(ctx, W, H); drawDotGrid(ctx, W, H);
  drawGlowOrb(ctx, W * 0.2, H * 0.3, 500, t.A, 0.15);
  drawGlowOrb(ctx, W * 0.8, H * 0.7, 450, t.B, 0.1);
  drawGrain(ctx, W, H, 3000, 0.04);
  const M = 40;
  fillRoundRect(ctx, M, M, W - M * 2, H - M * 2, 32, 'rgba(8,11,20,0.55)');
  strokeRoundRect(ctx, M, M, W - M * 2, H - M * 2, 32, 'rgba(255,255,255,0.06)', 1.5);
  const logo = await loadRemoteImage('https://hyperionsapplication.xyz/logo.png');
  if (logo) {
    ctx.save();
